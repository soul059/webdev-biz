...
// Define the interface for the receipt data
...
// -----------------------------------------------------------------
// Child Component: This is your original component's logic.
// It uses the hook and will be suspended by the parent.
// -----------------------------------------------------------------
function ReceiptViewer() {
  ...
  const urlReceiptId = searchParams?.get('id') // The hook is used here
  ...
  const fetchReceipt = async (id: string) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error('Receipt not found or an error occurred.')
      }
  ...
  // Use useEffect to fetch data when the component mounts with a URL ID
  useEffect(() => {
    if (urlReceiptId) {
      fetchReceipt(urlReceiptId)
    }
  }, [urlReceiptId]) // Dependency array ensures this runs only when urlReceiptId changes
          ...
          <div className="px-6 py-8">
                ...
                {receipt.projectDetails.technologies.length > 0 && (
                  <div className="mb-4">
                    ...
                    <div className="flex flex-wrap gap-2 mt-1">
                      {receipt.projectDetails.technologies.map(
                        (tech, index) => (
                          <span
                            key={index}
                ...
                {receipt.projectDetails.deliverables.length > 0 && (
                  <div>
                    ...
                    <ul className="list-disc list-inside mt-1 text-gray-600">
                      {receipt.projectDetails.deliverables.map(
                        (deliverable, index) => (
            ...
            <div className="border-t pt-6">
              <div className="flex justify-between items-center mb-4">
                ...
                >
                  {receipt.paymentInfo.status.charAt(0).toUpperCase() +
                    receipt.paymentInfo.status.slice(1)}
                </span>
              </div>
              ...
              <div className="grid md:grid-cols-2 gap-4">
                <div>
                  <p className="text-gray-600">
                    ...
                    {receipt.paymentInfo.currency}{' '}
                    {receipt.paymentInfo.amount.toLocaleString()}
                  </p>
                  <p className="text-gray-600">
            ...
            <div className="mt-8 pt-6 border-t text-center">
              <p className="text-sm text-gray-500">
                This is a digitally generated receipt. For any queries, please
                contact {receipt.freelancerInfo.email}
              </p>
            </div>
          </div>
        </div>
...
// -----------------------------------------------------------------
// Parent Component: This is the new page component.
// It wraps the child in <Suspense> and provides a fallback.
// This is what you should `export default`.
// -----------------------------------------------------------------
export default function ReceiptPage() {
  return (
    <div className="min-h-screen bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
      <Suspense fallback={<div className="text-center py-12">
            <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-blue-600 mx-auto"></div>
            <p className="mt-4 text-gray-600">Loading receipt from URL...</p>
          </div>}>
        <ReceiptViewer />
      </Suspense>
    </div>
